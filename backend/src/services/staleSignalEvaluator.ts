...
async function staleSignalValidator(): Promise<StaleSignal[]> {
  ...
  try {
    ...
    SELECT * FROM "Signal"
    WHERE array_length("rules", 1) IS DISTINCT FROM array_length("outcomes", 1);
`;
  } catch (error) {
...
async function fetchKlines(
  ...
  try {
    ...
    console.error(`Error fetching klines for symbol ${symbol}:`, error);
    return [];
  }
}
 ...
 * @returns Timestamp of first breach, or 0 if none found within max attempts
 *
 * Behavior:
...
async function checkCondition(
  ...
  let lastFoundTime: number | null = null;
    ...
    for (const ruleId of rules) {
      ...
      if (result !== 0) {
        ...
        const outcome: OutcomeType = {
          ...
          condition: result,
          result: 0,
          targetPrice: staleSignal.open * (1 + up / 100),
          stopPrice: staleSignal.open * (1 - down / 100),
          duration: 0,
